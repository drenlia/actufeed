 ...
 * We walk the DOM: one block per <p> (etc.). Inline <a> becomes markdown-style [label](url)
 * so the app can show tappable anchor text (not raw URLs).
 */
function markdownLinkFromAnchor(inner, abs) {
  let label = inner.replace(/[ \t\f\v\u00a0]+/g, ' ').replace(/\]/g, '').trim();
  if (!label) {
    try {
      label = new URL(abs).hostname.replace(/^www\./i, '');
    } catch {
      label = 'Link';
    }
  } else if (/^https?:\/\//i.test(label) || label === abs) {
    try {
      label = new URL(abs).hostname.replace(/^www\./i, '');
    } catch {
      /* keep label */
  ...
  function inlineFromNode(node) {
    ...
    if (tag === 'a') {
      ...
      try {
        ...
        return markdownLinkFromAnchor(inner, abs);
      } catch {
        return inner;
      }
    }
